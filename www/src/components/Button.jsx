...
export default function Button({ variant = 'action', children, className = '', ...props }) {
	if (variant === 'navl' || variant === 'navr') {
		return (
			<button type='button' className={`group flex size-6 items-center justify-center rounded-[40%] border border-(--color-border-secondary) bg-(--color-surface-white) transition-colors hover:border-(--color-border-primary) hover:bg-(--color-surface-action-hover) ${className}`} {...props} >
